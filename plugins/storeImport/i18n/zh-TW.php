...
return array(
    ...
    "storeImport.main.ioFromPathDesc" => "需要匯入的原始資料目錄，必須為網盤儲存下的目錄",
    "storeImport.main.ioFromPathErr" => "原始資料目錄錯誤，必須為儲存目錄！",
    "storeImport.main.ioToPath"      => "網盤存放目錄",
    "storeImport.main.ioToPathDesc"  => "需要存放的網盤系統目錄，如個人空間、企業網盤或其子目錄。建議為空目錄",
    "storeImport.main.ioToPathErr"   => "網盤存放目錄錯誤，必須為網盤系統目錄！",
    "storeImport.main.rootGroup"     => "企業網路磁碟",
    "storeImport.main.importDesc"    => "<h5>透過對磁碟或物件儲存進行掃描，自動建立索引，實現檔案快速匯入到網盤。</h5> \n<div class='mt-10 mb-20'> \n<li> 1. 網盤檔案需透過儲存路徑訪問，<u>因此在匯入之前，需將待匯入的原始資料目錄（或父級目錄）新增為儲存體</u>；</li> \n<li> 2. 此操作不會對原始資料做改動，只建構文件索引寫入網盤資料庫，<u>導入之後，不能對原始資料做任何改動，避免索引失效</u>；</li> \n<li> 3. 導入的資料與網盤預設儲存資料的路徑結構不同，建議將導入資料的儲存作為附加儲存（而非系統預設儲存）使用，以維護資料結構統一；</li> \n<li> 4. 建議在匯入之前對資料庫進行備份，以免發生意外。</li> \n<li> 5. 建議<u>單次導入資料量不要超過100萬條</u>，網盤存放目錄為空，避免資料過多導致記憶體溢位。</li> \n</div> \n<div>注意：<u>檔案路徑長度超過255個字元會被限制匯入</u>，清單記錄在網盤存放目錄下的「匯入失敗檔案(長度超255字元)」目錄中，可在匯入完成後檢視並自行處理。</div>",
    "storeImport.main.checkCnt"      => "檢查數量",
    "storeImport.main.selectFromPath" => "請選擇原始目錄",
    "storeImport.main.allCnt"        => "[0]個資料夾，[1]個文件",
    "storeImport.main.errCnt"        => "資料量過大，將會導致記憶體溢出，請務必分批導入",
    "storeImport.main.warnCnt"       => "資料量較大，建議分批導入",
    ...
    "storeImport.task.errLog"        => "導入失敗文件(長度超255字元)",
    "storeImport.task.importOK"      => "導入成功！",
    "storeImport.task.importEnd"     => "導入完成！",
    "storeImport.task.importErr"     => "導入失敗！",
    "storeImport.task.taskEnd"       => "任務結束！",
    "storeImport.task.taskErr"       => "任務異常！",
    "storeImport.task.reqErr"        => "請求失敗，或系統錯誤！",
    "storeImport.task.getting"       => "讀取中，請稍候",
    "storeImport.task.readdir"       => "讀取目錄",
    "storeImport.task.updateSize"    => "導入完成，正在更新目錄...",
    "storeImport.task.impFolder"     => "正在建置目錄...",
    "storeImport.task.impFile"       => "正在導入檔案...",
    "storeImport.task.notFinished"   => "未完成",
    "storeImport.task.errDesc"       => "手動或異常終止",
    "storeImport.task.partDesc"      => "部分文件導入失敗"
);