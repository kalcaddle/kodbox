...
<li>IP区间段: ip在区间内则匹配;例如:192.168.1.1-192.168.200.255</li>",
    ...
    "admin.setting.ifSaveDb"         => "数据库切换，会将系统当前的数据导入至新的数据库，并将其设为默认。<br>确定要执行吗？",
    ...
    "admin.setting.recDescInfo1"     => "通过将备份数据库文件中的数据写入至新的数据库，并将其设为系统默认，实现数据的恢复。",
    "admin.setting.recDescInfo2"     => "新的数据库配置参数，将追加到系统配置文件 config/setting_user.php，如执行恢复后系统出现异常，可去掉该文件中追加的部分，不影响此前的系统数据。",
    "admin.setting.recDescInfo3"     => "本功能仅支持处理由系统备份管理产生的备份数据，自行备份的数据库请通过其他方式处理。",
    ...
    "admin.share.rptIfBatch"         => "确定要进行此操作吗？",
