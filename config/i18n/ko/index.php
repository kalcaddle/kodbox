...
<li> IP 범위 : ip는 범위 내에서 일치합니다 (예 : 192.168.1.1-192.168.200.255).</li>",
    ...
    "admin.setting.ifSaveDb"         => "데이터베이스 스위치는 시스템의 현재 데이터를 새 데이터베이스로 가져와 기본값으로 설정합니다.<br> 실행하시겠습니까?",
    ...
    "admin.setting.recDescInfo1"     => "백업 데이터베이스 파일의 데이터를 새 데이터베이스에 쓰고 이를 시스템 기본값으로 설정하면 데이터 복구가 구현됩니다.",
    "admin.setting.recDescInfo2"     => "새로운 데이터베이스 설정 매개변수는 시스템 설정 파일 config/setting_user.php에 추가되며, 복구 실행 후 시스템이 비정상적일 경우 이전 시스템 데이터에 영향을 주지 않고 파일의 추가된 부분을 제거할 수 있습니다.",
    "admin.setting.recDescInfo3"     => "이 기능은 시스템 백업 관리에서 생성된 백업 데이터의 처리만 지원하며, 자신이 백업한 데이터베이스는 다른 방식으로 처리해야 합니다.",
...
<li> 업데이트 : 다운로드 후 해당 플러그인 이름 폴더에 업로드하고 압축을 풉니 다.</li>",
    ...
    "admin.share.rptIfBatch"         => "정말 하시겠습니까?",
