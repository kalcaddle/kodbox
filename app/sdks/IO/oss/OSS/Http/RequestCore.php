...
class RequestCore
{
   ...
   public function streaming_header_callback($curl_handle, $header_content)
   {
    ...
    public function streaming_read_callback($curl_handle, $file_handle, $length)
    {
        ...
        $size = min($this->read_stream_size - $this->read_stream_read, $length);
        if($size <= 0 ) return '';
        $read = fread($this->read_stream,$size); // Remaining upload data or cURL's requested chunk size
...
//		curl_setopt($curl_handle, CURLOPT_CLOSEPOLICY, CURLCLOSEPOLICY_LEAST_RECENTLY_USED);
    ...
    public function send_request($parse = false)
    {
        ...
        if ($this->response === false) {
            throw new RequestCore_Exception('cURL resource:cURL error: ' . curl_error($curl_handle) . ' (' . curl_errno($curl_handle) . ')');
        }
