...
function is_text_file($ext){
	...
	if(!$extArray){
		$extArray = array(
			...
			'applescript,access,aql,arcconfig,arclint,as,asc,asciidoc,asl,asm,asn,asn1,asp,aspx,ass,astylerc,atom,authors,aw,awk',
			...
			'd,dae,darglint,dart,dcurl,de,def,defs,depcomp,deploy,description,desktop,di,diff,dist,dockerfile,dockerfile-dist,dockerfile-master,dockerignore',
			...
			's,sample,sas,sass,sbt,scad,scala,schema,scheme,scm,sco,scss,scurl,sed,servers,service,security,settings,sgml,sh,sh_history,sharedmimeinfo,shtml,sieve,sig,siv',
