...
| content | text CHARACTER SET utf8    | 评论内容 |
...
| key | varchar(255) CHARACTER SET utf8    | 字段key |
| value | text CHARACTER SET utf8    | 字段值 |
| createTime | int(11) unsigned    | 创建时间 |
...
| name | varchar(255) CHARACTER SET utf8    | 群组名 |
| parentID | bigint(20) unsigned    | 父群组id |
| parentLevel | varchar(1000) CHARACTER SET utf8    | 父路径id; 例如:  ,2,5,10 |
| extraField | varchar(100) CHARACTER SET utf8 默认值=NULL  | 扩展字段 |
...
| key | varchar(255) CHARACTER SET utf8    | 存储key |
| value | text CHARACTER SET utf8    | 对应值 |
| createTime | int(11) unsigned    | 创建时间 |
| modifyTime | int(11) unsigned    | 最后修改时间 |
...
| name | varchar(255) CHARACTER SET utf8    | 文件名 |
| size | bigint(20) unsigned    | 文件大小 |
| ioType | int(10) unsigned    | io的id |
| path | varchar(255) CHARACTER SET utf8    | 文件路径 |
| hashSimple | varchar(100) CHARACTER SET utf8    | 文件简易hash(不全覆盖)；hashSimple |
| hashMd5 | varchar(100) CHARACTER SET utf8    | 文件hash, md5 |
...
| content | mediumtext CHARACTER SET utf8    | 文本文件内容,最大16M |
| createTime | int(11) unsigned    | 创建时间 |
...
| sourceHash | varchar(20) CHARACTER SET utf8    |  id的hash |
...
| name | varchar(256) CHARACTER SET utf8    | 文件名 |
| fileType | varchar(10) CHARACTER SET utf8    | 文件扩展名，文件夹则为空 |
| parentID | bigint(20) unsigned    | 父级资源id，为0则为部门或用户根文件夹，添加用户部门时自动新建 |
| parentLevel | varchar(2000) CHARACTER SET utf8    | 父路径id; 例如:  ,2,5,10 |
...
| type | varchar(255) CHARACTER SET utf8    | 事件类型 |
| desc | text CHARACTER SET utf8    | 数据详情，根据type内容意义不同 |
| createTime | int(11) unsigned    | 创建时间 |
...
| size | bigint(20)    | 文件大小 |
| detail | varchar(1024) CHARACTER SET utf8    | 版本描述 |
| createTime | int(11) unsigned    | 创建时间 |
| modifyTime | int(11) unsigned    | 最后修改时间 |
...
| sourceID | bigint(20) unsigned    | 文档id |
| userID | bigint(20) unsigned    | 操作者id |
| parentLevel | varchar(2000) CHARACTER SET utf8    | 文档上层关系;冗余字段,便于统计回收站信息 |
| createTime | int(11) unsigned    | 创建时间 |
...
| title | varchar(255) CHARACTER SET utf8    | 分享标题 |
| shareHash | varchar(50) CHARACTER SET utf8    | shareid |
| userID | bigint(20) unsigned    | 分享用户id |
| sourceID | bigint(20)    | 用户数据id |
| sourcePath | varchar(1024) CHARACTER SET utf8    | 分享文档路径 |
| url | varchar(255) CHARACTER SET utf8    | 分享别名,替代shareHash |
| isLink | tinyint(4) unsigned    | 是否外链分享；默认为0 |
| isShareTo | tinyint(4) unsigned    | 是否为内部分享；默认为0 |
| password | varchar(255) CHARACTER SET utf8    | 访问密码,为空则无密码 |
| timeTo | int(11) unsigned    | 到期时间,0-永久生效 |
| numView | int(11) unsigned    | 预览次数 |
| numDownload | int(11) unsigned    | 下载次数 |
| options | varchar(1000) CHARACTER SET utf8    | json 配置信息;是否可以下载,是否可以上传等 |
| createTime | int(11) unsigned    | 创建时间 |
| modifyTime | int(11) unsigned    | 最后修改时间 |
...
| desc | text CHARACTER SET utf8    | 举报原因（其他）描述 |
...
| ---- | ---- | ---- |
| id | bigint(20) unsigned   `自动增量` |
| sessionID | varchar(128) CHARACTER SET utf8    | session识别码，用于登陆时记录ip,UA等信息 |
| userID | bigint(20) unsigned    | 用户id |
| type | varchar(255) CHARACTER SET utf8    | 日志类型 |
| desc | text CHARACTER SET utf8    | 详情 |
| createTime | int(11) unsigned    | 创建时间 |
...
| type | varchar(50) CHARACTER SET utf8    | 配置类型 |
...
| sign | varchar(128) CHARACTER SET utf8    | session标识 |
| userID | bigint(20) unsigned    | 用户id |
| content | text CHARACTER SET utf8    | value |
...
| name | varchar(255) CHARACTER SET utf8    | 登陆用户名 |
| roleID | int(11) unsigned    | 用户角色 |
| email | varchar(255) CHARACTER SET utf8    | 邮箱 |
| phone | varchar(20) CHARACTER SET utf8    | 手机 |
| nickName | varchar(255) CHARACTER SET utf8    | 昵称 |
| avatar | varchar(255) CHARACTER SET utf8    | 头像 |
| sex | tinyint(4) unsigned    | 性别 (0女1男) |
| password | varchar(100) CHARACTER SET utf8    | 密码 |
| sizeMax | double unsigned    | 群组存储空间大小(GB) 0-不限制 |
| sizeUse | bigint(20) unsigned    | 已使用大小(byte) |
...
| name | varchar(255) CHARACTER SET utf8    | 收藏名称 |
| path | varchar(2048) CHARACTER SET utf8    | 收藏路径,tag时则为sourceID |
| type | varchar(20) CHARACTER SET utf8    | source/path |
| sort | int(11) unsigned    | 排序 |
| modifyTime | int(11) unsigned    | 最后修改时间 |
| createTime | int(11) unsigned    | 创建时间 |
...
| id | bigint(20) unsigned   `自动增量`  | 自增id |
| userID | bigint(20) unsigned    | 用户id |
| type | varchar(50) CHARACTER SET utf8    | 配置类型,全局配置类型为空,编辑器配置type=editor |
| key | varchar(255) CHARACTER SET utf8    | 配置key |
| value | text CHARACTER SET utf8    | 配置值 |
| createTime | int(11) unsigned    | 创建时间 |
| modifyTime | int(11) unsigned    | 最后修改时间 |
